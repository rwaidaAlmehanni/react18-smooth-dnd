...
class Container extends Component<ContainerProps> {
  ...
  isObjectTypePropsChanged(prevProps: ContainerProps) {
    ...
    for (const _key in containerOptions) {
      ...
      if (key && containerOptions.hasOwnProperty(key)) {
        const prop: any = containerOptions[key];

        if (prop && typeof prop !== 'function' && prop !== prevProps[key]) {
          return true;
        }
      }
    }
  ...
  getContainerOptions(): ContainerOptions {
    return Object.keys(this.props)?.reduce((result: ContainerOptions, key: string) => {
      ...
      if (prop && optionName && result[optionName] !== undefined && typeof prop === 'function') {
        (result as any)[optionName] = (...params: any[]) => {
          return (this.props[optionName] as Function)(...params);
        }
      } else if(prop && this.props[optionName]){
        result[optionName] = prop as any;
      }
